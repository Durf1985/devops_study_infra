...
project="$(gcloud projects list --format=json | jq '.[0] | .name' --raw-output)"
...
packer build -var-file="$SCRIPT_PATH"/variables.json  "$SCRIPT_PATH"/app.json
packer build -var-file="$SCRIPT_PATH"/variables.json  "$SCRIPT_PATH"/db.json
